...
bool AllTheSameLabel(vector <vector <string> > remain_state, string label){
	...
	for (unsigned int i = 0; i < remain_state.size(); i++){
		if (!remain_state[i][MAXLEN - 1].compare(label)) count++;//比较相同，compare返回值为0，如果所有比较值都相同，返回ture，否则为false
	}
...
#if 0
	string s;
	while (cin >> s, s.compare(endss) != 0){//-1为输入结束
		item[0] = s;
...
int main(){
	...
	Input();
	vector <string> remain_attribute;//定义个一个字符串容器

	string outlook("Outlook");//把字符串压进容器
	...
	ComputeMapFrom2DVector();//根据数据实例计算属性与值组成的map
